...
| Rich Markdown/diff views | Implemented (lightweight) | Tool payloads are retained for lazy expansion/export, ACP tool updates replace their original card, unified patches support inline or side-by-side views with line colors, and headings, fences, emphasis, lists, quotes, links, tables, rules, and source references are styled lazily. Full Textual widget/layout fidelity is intentionally outside the tmux performance contract. |
...
The Rust client is functionally complete for the exposed tmux-first relay and
