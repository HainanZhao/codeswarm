...
| Rich Markdown/diff views | Partial | Tool payloads are retained for lazy expansion/export, unified patches support inline or side-by-side views with line colors, and headings, fences, emphasis, lists, quotes, links, tables, rules, and source references are styled lazily; full Markdown layout remains intentionally unported. |
| OS notifications/sounds | Partial | Rust preserves Python's `blur`/`always`/`never` policy (plus legacy boolean settings), emits completion and permission-request notifications through `notify-send`/`osascript`, supports a sanitized OSC terminal title with reference-counted blinking alerts, and persists the blink toggle. Permission requests use the sound toggle with a portable terminal BEL (the Python `question.wav` is 658 KB and requires `notifypy`/platform audio integration); turn-over notifications intentionally remain silent as in Python. Named sound assets are not shipped to keep the Rust client lightweight. |
