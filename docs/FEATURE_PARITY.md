...
| Prompt path/resource completion | Partial | Rust has a shared bounded, root-safe resource loader, ACP text/binary attachment expansion, and bounded `@path` fuzzy completion; the Python-style asynchronous index/preview UI remains unported. |
| Live roster reconfiguration | Partial | Rust supports a catalog-backed `/config` roster editor for the next launch plus live `/add`, `/drop SLOT`, `/promote SLOT`, `/swap A B`, reload, stable slots, and healthy-peer continuation; Python-style in-session catalog reconciliation remains unported. |
...
| Persistent prompt/settings preferences | Partial | Roster, custom agents, Python-compatible prompt history, follow-tail, collapsed-details, notification policy, thoughts, tool-expansion, density, scrollbar, and diff view persist; theme/focus preferences do not. |
...
| OS notifications/sounds | Partial | Rust preserves Python's `blur`/`always`/`never` policy (plus legacy boolean settings), emits completion and permission-request notifications through `notify-send`/`osascript`, and gates terminal bells with the sound toggle; title blinking and named sound assets are not yet ported. |
