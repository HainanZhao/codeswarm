...
| Live roster reconfiguration | Partial | Rust supports a catalog-backed `/config` editor with idle-session add/drop/promote/swap reconciliation, next-launch persistence, and live `/add`, `/drop SLOT`, `/promote SLOT`, `/swap A B`; edits are deferred when a new owner is not already live and the full Python catalog/session reconciliation workflow is not yet complete. |
