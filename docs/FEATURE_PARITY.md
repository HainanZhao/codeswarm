...
| Prompt path/resource completion | Partial | Rust has a bounded, root-safe asynchronous workspace index, `.gitignore` filtering, fuzzy `@path` popup with directory/quoted insertion, ACP text/binary attachment expansion, and stale-generation protection; mouse dismissal and a full Python picker surface remain unported. |
