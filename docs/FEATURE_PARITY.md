...
| Live roster reconfiguration | Partial | Rust edits the saved roster through the store, continues with a healthy peer after failure/drop, and can reload failed slots; the Python conversation config could reconcile roster additions, removals, and owner transfer in place. |
