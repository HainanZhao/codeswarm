...
| OS notifications/sounds | Partial | Optional `notify-send`/`osascript` completion notifications are emitted only while the terminal reports focus lost, and the terminal bell is available from `/config`; named sound assets are not yet ported. |
