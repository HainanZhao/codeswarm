...
from codeswarm.acp import protocol
from codeswarm.acp import api
from codeswarm.acp.api import API
from codeswarm.acp import messages
from codeswarm.acp.prompt import build as build_prompt
from codeswarm.db import DB, decode_session_meta
from codeswarm import paths
from codeswarm import constants
...
LOG_TRUNCATED_MESSAGE = "[codeswarm] log truncated after 1 MiB"
RESPONSE_TRUNCATED_MESSAGE = (
    "\n\n[CodeSwarm stopped rendering the rest of this unusually long response.]\n"
)
THOUGHT_TRUNCATED_MESSAGE = (
    "\n\n[CodeSwarm stopped rendering the rest of this unusually long thought.]\n"
)
OPERATING_INSTRUCTIONS = """\
## CodeSwarm operating instructions
...
def _parse_node_version(value: str) -> tuple[int, int, int] | None:
    """Parse the numeric core of a Node version such as ``v22.18.0``."""
...
def _read_node_version(node: Path) -> tuple[int, int, int] | None:
    """Read a Node executable's version without involving the user's shell."""
    try:
        ...
        return None
    return _parse_node_version(result.stdout) if result.returncode == 0 else None


def _prepare_node_environment(
    env: dict[str, str], minimum_version: str
) -> dict[str, str]:
    """Prefer a compatible installed NVM runtime for a Node-based adapter."""
    minimum = _parse_node_version(minimum_version)
    if minimum is None:
        raise RuntimeError(f"Invalid minimum Node.js version: {minimum_version}")

    path_value = env.get("PATH", "")
    current_name = shutil.which("node", path=path_value)
    current = Path(current_name) if current_name is not None else None
    current_version = _read_node_version(current) if current is not None else None
    if current_version is not None and current_version >= minimum:
        return env

    nvm_root = Path(env.get("NVM_DIR", str(Path.home() / ".nvm")))
    candidates = list((nvm_root / "versions" / "node").glob("*/bin/node"))
    compatible = [
        (version, node)
        for node in candidates
        if (version := _read_node_version(node)) is not None and version >= minimum
    ]
    if not compatible:
        current_label = (
        ...
        raise RuntimeError(
            f"This adapter requires Node.js {minimum_version} or newer "
            f"(current: {current_label})"
        )

    _version, selected = max(compatible, key=lambda candidate: candidate[0])
    prepared = env.copy()
    # An empty PATH element resolves to the working directory in a POSIX shell.
    prepared["PATH"] = os.pathsep.join(
        part for part in (str(selected.parent), path_value) if part
    )
    return prepared
...
class Agent(AgentBase):
    ...
    def __init__(
        ...
        self._full_access_startup_argument = agent.get(
            "full_access_startup_argument"
        )
        self._startup_full_access = bool(self._full_access_startup_argument)
        ...
        if log_path := os.environ.get("CODESWARM_LOG"):
    ...
    def command(self) -> str | None:
        ...
        acp_command = codeswarm.get_os_matrix(self._agent_data["run_command"])
        if (
            acp_command is not None
            and self._startup_full_access
            and (argument := self._full_access_startup_argument)
        ):
            return f"{acp_command} {shlex.quote(argument)}"
        return acp_command

    @property
    def supports_startup_full_access(self) -> bool:
        return bool(self._full_access_startup_argument)

    @property
    def startup_full_access(self) -> bool:
        return self.supports_startup_full_access and self._startup_full_access

    def configure_startup_full_access(self, enabled: bool) -> None:
        if enabled and not self.supports_startup_full_access:
    ...
    async def rpc_terminal_output(
        ...
        from codeswarm.widgets.terminal_tool import ToolState
    ...
    async def _run_agent(self) -> None:
        ...
        env = os.environ.copy()
        env["CODESWARM_CWD"] = str(self.project_root_path)
        if minimum_node := self._agent_data.get("minimum_node_version"):
            try:
                # Each probe shells out to `node --version`; keep the
                # blocking work off the UI's event loop.
                env = await asyncio.to_thread(
                    _prepare_node_environment, env, minimum_node
                )
            except RuntimeError as error:
                self.post_message(
                    AgentFail("Failed to start agent", details=str(error))
                )
                return
        if self._agent_data["identity"] == "geminicli.com":
            ...
            # Keep CodeSwarm's adapter process isolated from that optional path.
