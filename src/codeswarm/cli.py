...
async def get_agent_data(launch_agent) -> Agent | None:
    from codeswarm.agents import resolve_agent
...
def _read_saved_roster_identities() -> list[str]:
    ...
    Read directly off disk rather than through `Settings`/`CodeSwarmApp`: this
    ...
    settings_path = paths.get_config() / "codeswarm.json"
...
def main(ctx, version):
    """CodeSwarm — AI for your terminal."""
    if version:
        from codeswarm import get_version
...
def run(
    ...
    if agents:
        ...
        setup_prompt = not (paths.get_config() / "codeswarm.json").exists()
...
def acp(
    ...
    agent_data: AgentData = {
        ...
        "short_name": "agent",
        "url": "https://github.com/HainanZhao/codeswarm",
        "protocol": "acp",
        "type": "coding",
    ...
    app = CodeSwarmApp(agent_data=agent_data, project_dir=project_dir)
    app.run()
