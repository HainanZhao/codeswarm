...
type AgentProtocol = Literal["acp", "native"]
"""The transport used to communicate with the agent."""
...
class Agent(TypedDict):
    """Describes an agent which CodeSwarm can connect to.
    ...
    """Alternative short names or aliases for the agent."""
    url: str
    """A URL for the agent."""
    protocol: AgentProtocol
    """The protocol used by the agent: an ACP adapter or native integration."""
    type: "AgentType"
    ...
    minimum_node_version: NotRequired[str]
    """Minimum Node.js runtime for an adapter launched through Node tooling."""
    full_access_startup_argument: NotRequired[str]
    """Process argument that enables genuine permission bypass for this adapter."""
    detect_command: NotRequired[dict[OS, str]]
    """Command whose executable proves the agent is locally available.

    This may differ from ``run_command`` for adapters launched through a
