...
from codeswarm.session_tracker import SessionTracker
...
# A pure-black terminal palette with CodeSwarm's cool avionics accents.
CODESWARM_TERMINAL_THEME = terminal_theme.TerminalTheme(
...
CODESWARM_BLACK_THEME = Theme(
    name="codeswarm-black",
    primary="#2DD4BF",
    secondary="#14B8A6",
    warning="#A78BFA",
    error="#EF4444",
...
class CodeSwarmApp(App, inherit_bindings=False):
    """The top level app."""

    CSS_PATH = "codeswarm.tcss"
    ...
    terminal_title: var[str] = var("CodeSwarm")
    ...
    def settings_path(self) -> Path:
        return paths.get_config() / "codeswarm.json"
    ...
    def copy_finished_selection(self) -> None:
        ...
        selection useful regardless of which terminal hosts CodeSwarm.
        """
    ...
    def system_notify(
        ...
        notification.application_name = "CodeSwarm"
        if sound and self.settings.get("notifications.enable_sounds", bool):
            sound_path = str(files("codeswarm.data").joinpath(f"sounds/{sound}.wav"))
    ...
    def setting_updated(self, key: str, value: object) -> None:
        if key == "ui.theme":
            self.theme = CODESWARM_BLACK_THEME.name
    ...
    async def on_load(self) -> None:
        self.register_theme(CODESWARM_BLACK_THEME)
        ...
        if (
            ...
            and ui_settings.get("theme") != CODESWARM_BLACK_THEME.name
        ):
            ui_settings["theme"] = CODESWARM_BLACK_THEME.name
            settings_migrated = True
        if isinstance(ui_settings, dict) and ui_settings.get("scrollbar") == "thin":
            ui_settings["scrollbar"] = "normal"
            settings_migrated = True
        self.ansi_theme_dark = CODESWARM_TERMINAL_THEME
    ...
    async def replace_live_conversations(self) -> None:
        ...
        CodeSwarm presents one active workspace. Returning to the agent store is
    ...
    def set_process_title(self) -> None:
        try:
            ...
            setproctitle.setproctitle("codeswarm")
        except Exception:
            pass
    ...
    def get_main_screen(self) -> MainScreen:
        ...
        ).data_bind(scrollbar=CodeSwarmApp.scrollbar)

    async def action_interrupt_or_quit(self) -> None:
        """Follow terminal convention: interrupt work before exiting CodeSwarm."""
    ...
    async def _launch_agent(
        ...
        *,
        ...
        from codeswarm.agents import AgentReadError, read_agents
