...
from codeswarm.shell_read import shell_read
...
class TerminalTool(Terminal):
    ...
    def __init__(
        ...
        *,
        ...
        # ACP permits adapters to omit this value. Keep a bounded transcript
        # in that case: terminal commands can otherwise grow the UI process
        # without limit merely by streaming output.
        if output_byte_limit is not None and output_byte_limit < 0:
            raise ValueError("output_byte_limit must not be negative")
        self._output_byte_limit = min(
            output_byte_limit
    ...
    async def start(self, width: int = 0, height: int = 0) -> None:
        ...
        if self._start_error is not None:
            raise RuntimeError("Unable to start agent terminal") from self._start_error

    async def run(self) -> None:
        try:
            await self._run()
        except Exception as error:
            # The ACP handler reports this cleanly to the agent. Never leak a
            # traceback into the user's terminal or strand start() waiting.
    ...
    def get_output(self) -> tuple[str, bool]:
        ...
        # Chunks may already have been evicted in ``_record_output``. Compare
        # against all bytes observed, not only the retained deque, so ACP
        # callers never mistake a partial transcript for complete output.
