...
from textual.widgets import OptionList, Static, Select
...
class PermissionsQuestion(Question):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.set_reactive(Question.selection, -1)
    ...
    def action_select(self) -> None:
        if self.selection < 0:
            self.notify(
                "Choose a permission option before confirming",
                title="Permission",
                severity="warning",
            )
            return
...
class PermissionsScreen(Screen[Answer]):
    ...
    async def add_diff(
        ...
        from wingmen.widgets.diff_view import make_diff
