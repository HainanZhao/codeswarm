...
from wingmen.pill import pill
from wingmen import messages
from wingmen.widgets.directory_input import DirectoryInput
from wingmen.widgets.condensed_path import CondensedPath
from wingmen.widgets.grid_select import GridSelect
...
class StoreScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield widgets.Input(placeholder="Filter agents…", id="agent-filter")
        yield widgets.Static(id="roster-strip", classes="-empty")
        yield Container(id="container", can_focus=False)

    def get_info(self) -> Content:
        return Content.assemble(
            Content.from_markup("Wingmen"),
            pill(f"v{wingmen.get_version()}", "$primary-muted", "$text-primary"),
    ...
    def _matches_filter(self, agent: Agent) -> bool:
        if not self._filter_text:
            return True
        haystack = f"{agent['name']} {agent['description']}".casefold()
    ...
    def compose_agents(self) -> ComposeResult:
        ...
        ordered_agents = sorted(
            (
                ...
                if agent["type"] == "coding" and self._matches_filter(agent)
            ),
    ...
    def on_agent_grid_select_launch_roster(
        ...
        if self._roster_selection:
            ...
            if unavailable:
                self.notify(
                    "Not detected: "
                    f"{', '.join(unavailable)}. Install the CLI and reopen Wingmen.",
        ...
        if message.identity is not None and message.identity in self._installed:
            ...
            self.notify(
                f"{name} is not detected. Install the CLI and reopen Wingmen.",
                title="Agent unavailable",
                severity="warning",
            )
    ...
    @on(widgets.Input.Changed, "#agent-filter")
    async def on_agent_filter_changed(self, event: widgets.Input.Changed) -> None:
        self._filter_text = event.value.strip().casefold()
