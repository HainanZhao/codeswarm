...
class FuzzyIndex:
    ...
    async def search(self, query: str) -> list[str]:
        """Search without blocking the Textual event loop."""
        async with self._lock:
            return await asyncio.to_thread(self._search, query)

    @staticmethod
    def _score_ordered_match(
        path: str, query: str, weights: dict[int, float]
    ) -> float | None:
        """Score a linear, ordered fuzzy match for candidate prioritization.

        The full path picker later evaluates every possible match in its
        smaller candidate set. Here we only need a fast, order-aware score so
        strong matches survive the candidate cap. A character-count check is
        insufficient: ``"ab"`` contains the letters in ``"ba"`` but is not
        a fuzzy match for that query.
        """
        positions: list[int] = []
        start = 0
        for character in query:
            position = path.find(character, start)
            if position == -1:
                return None
            positions.append(position)
            start = position + 1

        score = sum(weights.get(position, 0.0) for position in positions)
        score += sum(
            1.0
        ...
        score += sum(
            ...
            if position == 0 or path[position - 1] == "/"
        )
        ...
        # Prefer compact matches when all other signals are equal.
        score -= (positions[-1] - positions[0] - len(query) + 1) * 0.05
        return score

    def _search(self, query: str) -> list[str]:
        """Perform the CPU-bound portion of :meth:`search`."""
        ...
        for path, normalized_path in self._find_candidates(normalized_query):
            ...
            score = self._score_ordered_match(
                normalized_path, normalized_query, weights
            )
            if score is None:
                continue
